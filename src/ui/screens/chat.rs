...
pub fn view<'a>(
    ...
    let content: Element<'_, AppMessage> = if let Some(preview) = &chat.image_preview {
        let image_display: Element<'_, AppMessage> = if !preview.image_path.is_empty() {
            image(preview.image_path.clone())
                .width(Length::Fill)
                .height(Length::Fill)
            ...
            container(
                text("加载中...")
                    .size(16)
                    .color(Color::from_rgb8(0xAF, 0xB6, 0xC1)),
            )
            .width(Length::Fill)
            .height(Length::Fill)
        ...
        // 下载原图进度提示
        let loading_hint: Element<'_, AppMessage> = if preview.loading_original {
            container(
                container(
                    text("正在加载原图...")
                        .size(13)
                ...
                .padding([6, 14])
                .style(|_| container::Style {
                    background: Some(Background::Color(Color::from_rgba8(0, 0, 0, 0.7))),
            ...
            .width(Length::Fill)
            .align_x(alignment::Horizontal::Center)
            .padding(iced::Padding { top: 0.0, right: 0.0, bottom: 50.0, left: 0.0 })
            .height(Length::Fill)
        ...
        // 关闭按钮
        let close_btn: Element<'_, AppMessage> = container(
            button(
                text("X").size(18).color(Color::WHITE),
            )
            .padding([6, 12])
        ...
        .width(Length::Fill)
        .align_x(alignment::Horizontal::Right)
        .padding(iced::Padding { top: 16.0, right: 16.0, bottom: 0.0, left: 0.0 })
        .into();
        ...
        stack![
            ...
            mouse_area(
                container(text(""))
                    ...
                    .height(Length::Fill)
                    .style(|_| container::Style {
                        background: Some(Background::Color(Color::from_rgba8(0, 0, 0, 0.85))),
                        ..container::Style::default()
                    })
            )
            .on_press(AppMessage::CloseImagePreview),
